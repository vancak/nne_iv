...
#    coef_gam <- coef(glm(A ~ Z, family = "binomial"))
    ...
    Y        <- rbinom(j, 1, prob = plogis( beta[1] + beta[2]*A + beta[3]*Z + beta[4]*I(A*Z) ))
    
    coef_bet <- coef(glm(Y ~ A + Z + I(A*Z), family = "binomial"))
...
#    pb          <- pb_fun(beta = coef_bet, gamma = coef_gam, psi = c(psi_u, psi_e), pz = p_z)

    coef_psi <- c(psi_u, psi_e)
    
    pb <- mean( plogis(coef_bet[1] + coef_bet[2]*A + coef_bet[3]*Z + coef_bet[4]*A*Z + coef_psi[1]*(1 - A)) - 
    ...
    p_ein1     <- sum( A * pnorm(coef_bet[1] + coef_bet[2] * A + coef_bet[3] * Z + coef_bet[4] * I(A*Z)) ) / sum(A)
...
apply(EIN_psi, 2, FUN = function(x) {sum(x < 1 | x > 1000, na.rm = T)})

NNE
write.csv(NNE_psi, "NNE_psi.csv", row.names = F)
write.csv(NNE_undj, "NNE_undj.csv", row.names = F)

apply(NNE_psi, 2, FUN = function(x) {sum(x < 1 | x > 1000, na.rm = T)})

NNT
write.csv(NNT_psi, "NNT_psi.csv", row.names = F)
write.csv(NNT_undj, "NNT_undj.csv", row.names = F)

apply(NNT_psi, 2, FUN = function(x) {sum(x < 1 | x > 1000, na.rm = T)})

colMeans(psi_mat, na.rm = T)
EIN
colMeans(EIN_psi[EIN_psi > 1,], na.rm = T)
colMeans(EIN_undj)
NNE
colMeans(NNE_psi[NNE_psi > 1,], na.rm = T)
colMeans(NNE_undj)
NNT
colMeans(NNT_psi[NNT_psi > 1,], na.rm = T)
colMeans(NNT_undj)
...
### SD vectors 
sd_NNT <- apply(NNT_psi[NNT_psi < 1000 & NNT_psi > 1,],  2, sd, na.rm = T)
sd_NNE <- apply(NNE_psi[NNE_psi < 1000 & NNE_psi > 1,],  2, sd, na.rm = T)
sd_EIN <- apply(EIN_psi[EIN_psi < 1000 & EIN_psi > 1,],  2, sd, na.rm = T)

sd_mat <- data.frame(rbind(sd_EIN, sd_NNE, sd_NNT))
t(sd_mat)
...
avb_NNT <- apply(NNT_psi[NNT_psi < 1000 & NNT_psi > 0,], 2, FUN = av_bias, y = NNT)
avb_NNE <- apply(NNE_psi[NNE_psi < 1000 & NNE_psi > 0,], 2, FUN = av_bias, y = NNE)
avb_EIN <- apply(EIN_psi[EIN_psi < 1000 & EIN_psi > 0,], 2, FUN = av_bias, y = EIN)

avb_mat <- data.frame(rbind(avb_EIN, avb_NNE, avb_NNT))
t(avb_mat)

### count irrelevant point estimators
nrow( NNT_psi[NNT_psi > 1000 | NNT_psi < 1, -ncol(NNT_psi)])
nrow( NNE_psi[NNE_psi > 1000 | NNE_psi < 1, -ncol(NNT_psi)])
nrow( EIN_psi[EIN_psi > 1000 | EIN_psi < 1, -ncol(NNT_psi)])
...
### CSV FILES ###
## SE
write.csv(sd_mat, "sd_LOGIT_m1000.csv", row.names = T)

## AVERAGE BIAS
write.csv(avb_mat, "avb_LOGIT_m1000.csv", row.names = T)

## CIs
write.csv(NNE_psi_CI, "NNE_psi_CI_LOGIT_m1000.csv", row.names = F)
write.csv(EIN_psi_CI, "EIN_psi_CI_LOGIT_m1000.csv", row.names = F)
write.csv(NNT_psi_CI, "NNT_psi_CI_LOGIT_m1000.csv", row.names = F)

### COVERAGE MATRIX - CREATION ###
NNE_psi_CI <- read.csv("NNE_psi_CI_LOGIT_m1000.csv")
EIN_psi_CI <- read.csv("EIN_psi_CI_LOGIT_m1000.csv")
NNT_psi_CI <- read.csv("NNT_psi_CI_LOGIT_m1000.csv")
...
round(t(rbind(COVER_EIN, COVER_NNE, COVER_NNT)), 3)
