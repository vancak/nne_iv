...
library(nleqslv)
library(pracma)
source("dh_functions_logit_V2.R")
source("py_&_pb_functions_logit_V2.R")
source("sandwich_matrix_logit_clean_v_V2.R")
...
### SD vectors 
sd_NNT <- apply(NNT_psi[NNT_psi < 1000 & NNT_psi > 1,-ncol(NNT_psi)],  2, sd, na.rm = T)
sd_NNE <- apply(NNE_psi[NNE_psi < 1000 & NNE_psi > 1,-ncol(NNE_psi)],  2, sd, na.rm = T)
sd_EIN <- apply(EIN_psi[EIN_psi < 1000 & EIN_psi > 1,-ncol(EIN_psi)],  2, sd, na.rm = T)
...
write.csv(bbb_both2, "NNTm1000_LOGIT_8.csv", row.names = F)
...
write.csv(bbb_both2, "EINm1000_LOGIT_6_408.csv", row.names = F)
...
write.csv(bbb_both2, "NNEm1000_LOGIT_12_771.csv", row.names = F)